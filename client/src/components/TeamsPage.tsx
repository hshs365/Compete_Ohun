  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      try {
        ...
        console.error('용병 크루 목록 로드 실패:', error);
        setTeams([]);
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto w-full pb-12">
      <div className="mb-4 flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">용병 크루</h2>
        <Link
          to="/teams/create"
      ...
      {isLoading ? (
        ...
        <div className="bg-[var(--color-bg-card)] rounded-2xl border border-[var(--color-border-card)] p-8 text-center">
            ...
            <>
              ...
              {NEARBY_REGIONS[userRegion] && NEARBY_REGIONS[userRegion].length > 0 && (
                <div className="mt-8 pt-6 border-t border-[var(--color-border-card)]">
                  <p className="text-sm text-[var(--color-text-secondary)] mb-3">근처 지역 용병 크루도 살펴보기</p>
          ...
          {activeTab === 'other' && !searchQuery.trim() && (
            <p className="text-sm text-[var(--color-text-secondary)]">위 검색창에 용병 크루명 또는 지역을 입력해 보세요.</p>
          )}
        </div>
      ) : (
