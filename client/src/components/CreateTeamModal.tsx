...
const CreateTeamModal: React.FC<CreateTeamModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await showError(err?.message || '용병 클럽 생성에 실패했습니다.', '용병 클럽 생성 실패');
    } finally {
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          {/* 용병 클럽 로고 */}
          <div>
            <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-2">용병 클럽 로고</label>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--color-text-primary)] mb-2">용병 클럽 소개</label>
            <textarea
              value={description}
