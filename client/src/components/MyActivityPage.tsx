  ...
  if (isLoading) {
    return (
      <div className="flex flex-1 w-full bg-[var(--color-bg-primary)] items-center justify-center min-h-[320px]">
        <LoadingSpinner fullScreen={false} message="매치 기록을 불러오는 중..." />
      </div>
    );
  }
          ...
          <div className="p-6">
            {!hasDataForCategory(categoryFilter) ? (
              <div className="py-16 text-center text-[var(--color-text-secondary)]">
                ...
                <p className="font-medium text-[var(--color-text-primary)]">해당 종목의 매치 기록이 없습니다.</p>
                <p className="text-sm mt-1">축구, 풋살, 농구, 테니스 매치에 참여해 보세요.</p>
              </div>
            ) : (
              <>
                  ...
                  {(
                    ...
                    return (
                      <button
                        ...
                        className={`group flex flex-col p-5 rounded-xl border transition-all text-left hover:-translate-y-1 hover:shadow-lg ${bgClass} ${borderClass}`}
                      >
